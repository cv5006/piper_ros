...
버튼은 세 묶음이다.
    ...
    IK 해             두 스택 어디서나 된다 — ik_solutions 는 URDF 만 읽는다.
                    초록과 빨강을 따로 껐다 켠다
    MoveIt 스택      장애물. move_group 이 있어야 한다
    manipulability     자세 프리셋과 타원체. pose_presets · manipulability 가 있어야 한다

두 스택은 같이 띄우지 않는다. 지금 떠 있지 않은 쪽 묶음은 **회색으로 잠겨** 있어서
누를 수 없다 (1 초마다 서비스 존재를 확인한다).

**일은 항상 딴 스레드에서 한다.** 버튼 콜백에서 그대로 기다리면 그동안 창이 얼어붙는데,
ik_solutions 는 몇 초가 걸리므로 강의 중에 창이 죽은 것처럼 보였다. 지금은 눌린 동안 버튼이
