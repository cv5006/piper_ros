...
지금 손끝 자세를 만드는 관절값을 전부 찾는다. 강의 반복식 그대로 —
q <- q + J⁺(x_target − f(q)), 무작위 시드, DLS 감쇠. **관절 한계는 걸지 않는다.**

    /ik_solutions/solve        Trigger   다시 푼다 (상주 노드)
    /ik_solutions/show_valid   SetBool   초록 = 한계 안
    /ik_solutions/reveal       SetBool   빨강 = 한계 밖

기본은 둘 다 감추기. 드러낼 때 다시 풀지 않는다. 자세를 옮기면 빨강은 되감긴다.

⚠ 초록 하나는 지금 로봇이 있는 그 자세다. 겹쳐 그려져 화면에 변화가 없다.
⚠ MoveIt 을 쓰지 않는다. Trajectory 디스플레이가 SRDF 를 요구해 URDF 스택에서
  뜨지 못한다. 링크 메시를 Marker 로 직접 놓는다.
⚠ 이 팔은 8해 중 쓸 수 있는 것이 거의 항상 1개다 (한계가 손목을 막는다).
  「해가 여럿」은 기구학, 「하나만 쓸 수 있다」는 이 로봇의 성질이다. 상세는 NOTES.md.
"""
...
def solve_ik(chain, Td, q0, iters=250, lam=0.05,
    ...
    """강의 반복식 그대로. **관절 한계는 걸지 않는다** — 못 쓰는 해를 보이는 것이 요지다.

    두 군데서 일찍 끊는다.

    ① tol 에서 수렴 정지 (예전 1e-11 은 채택 기준 1e-6 보다 빡빡해 안 걸렸다)
    ② stall_at 회까지 stall_tol 밖이면 포기 (시드 3 할이 끝내 수렴 못 하면서 시간 절반을 먹었다)

    무작위 6 자세 대조 — 40 시드로도 400 시드와 **해집합이 같고** 4.9 s -> 0.4 s."""
    q = q0.copy()
